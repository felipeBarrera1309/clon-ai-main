...
interface ComboAvailabilitySheetProps {
  open: boolean
...
export const ComboAvailabilitySheet = ({
  open,
  onOpenChange,
  combo,
}: ComboAvailabilitySheetProps) => {
  const { activeOrganizationId } = useOrganization()
  const locations = useQuery(
    api.private.restaurantLocations.list,
    activeOrganizationId ? { organizationId: activeOrganizationId } : "skip"
  )
  const comboAvailabilities = useQuery(
    api.private.comboAvailability.getByCombo,
    combo && activeOrganizationId
      ? {
  ...
  const setAvailability = useMutation(api.private.comboAvailability.set)
  ...
  const handleToggleAvailability = async (
    ...
    const toggleKey = `${comboId}-${locationId}`

    setTogglingAvailability((prev) => {
      const newMap = new Map(prev)
      newMap.set(toggleKey, true)
      return newMap
    })
    ...
    try {
      ...
      toast.success(
        currentAvailability
          ? "Combo removido de la sucursal"
          : "Combo agregado a la sucursal"
      )
    } catch (error) {
