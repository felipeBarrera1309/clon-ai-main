...
interface ComboFileUploadProps {
  onFilesSelected: (files: File[]) => void
  isUploading: boolean
}

export function ComboFileUpload({
  onFilesSelected,
  isUploading,
}: ComboFileUploadProps) {
  ...
  const validateAndAddFiles = useCallback((newFiles: FileList | File[]) => {
    ...
    for (const file of fileArray) {
      if (!isValidFileType(file)) {
        toast.error(
          `"${file.name}" no es un tipo válido. Solo PDF e imágenes (JPG, PNG).`
        )
        continue
      }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="mb-3 text-center text-muted-foreground text-xs">
          PDF o imágenes de tus combos (máx. {MAX_FILES} archivos,{" "}
          {MAX_FILE_SIZE_MB}
          MB cada uno)
        </p>
