...
const whatsappConfigSchema = z
  .object({
    ...
    gupshupAppId: z.string().optional(),
  ...
  .refine(
    (data) => {
      ...
      if (data.provider === "twilio") {
        return !!data.twilioAccountSid && !!data.twilioAuthToken
      }
...
export default function WhatsAppSettingsPage() {
  ...
  const whatsappConfigs = useQuery(
    api.superAdmin.whatsappConfiguration.getConfigurations,
    { organizationId }
  )
  // Mutations for WhatsApp configurations
  const createWhatsappConfig = useMutation(
    api.superAdmin.whatsappConfiguration.createConfiguration
  )
  const updateWhatsappConfig = useMutation(
    api.superAdmin.whatsappConfiguration.updateConfiguration
  )
  const deleteWhatsappConfig = useMutation(
    api.superAdmin.whatsappConfiguration.deleteConfiguration
  )
  const toggleConfigActiveStatus = useMutation(
    api.superAdmin.whatsappConfiguration.toggleActiveStatus
  )
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <Input placeholder="5491122334455" {...field} />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
